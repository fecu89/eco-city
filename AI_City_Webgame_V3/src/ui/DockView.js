...
export function renderDock() {
  ...
  orderedFacilities().forEach(([key, f]) => {
    ...
    btn.setAttribute('aria-disabled', String(locked || unaffordable));
    ...
    btn.innerHTML = `
      <div class="facility-card-main"><span class="f-icon">${f.icon}</span><span class="facility-card-identity"><strong>${f.name}</strong><span class="cost">-${formatCredits(f.cost)}</span></span></div>
    `;
    ...
    btn.addEventListener('click', () => {
      ...
      if (locked || unaffordable) {
        eventBus.emit(Events.TOAST_SHOW, {
          title: `${f.name} 건설 불가`,
          text: locked ? facilityUnlockMessage(gameState, key) : `${formatCredits(f.cost - gameState.credits)}가 더 필요합니다.`,
        });
        return;
      }
      selectFacility(key);
