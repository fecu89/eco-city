...
function readPinned() {
  try {
    return Boolean(JSON.parse(readStorage(QUEST_PANEL_LAYOUT.STORAGE_KEY) || '{}').pinned);
  } catch {
    return false;
  }
}
...
function savePinned(pinned) {
  let stored = {};
  try {
    stored = JSON.parse(readStorage(QUEST_PANEL_LAYOUT.STORAGE_KEY) || '{}');
  } catch {
    stored = {};
  }
  writeStorage(QUEST_PANEL_LAYOUT.STORAGE_KEY, JSON.stringify({ ...stored, pinned }));
}
