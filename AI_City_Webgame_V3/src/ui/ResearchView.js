...
function activeJobMarkup(job, dataCenterIndex) {
  ...
  return `
    <article class="research-active ${['underpowered', 'upgrade_paused'].includes(job.status) ? 'underpowered' : ''}" data-research-job="${job.id}">
...
export function refreshResearchPanelLive(root) {
  ...
  root.querySelectorAll('[data-research-job]').forEach((article) => {
    ...
    const definition = RESEARCH[job.id];
    article.classList.toggle('underpowered', ['underpowered', 'upgrade_paused'].includes(job.status));
