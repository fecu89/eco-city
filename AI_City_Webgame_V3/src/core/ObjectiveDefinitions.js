import { EVENT_FORECAST_DAYS } from './EventDefinitions.js';

const card = (id, category, title, description, durationDays = 0) => Object.freeze({
  id,
  category,
  title,
  description,
...
export const OBJECTIVE_SETS = Object.freeze({
  'transition-choice': Object.freeze({
    ...
    required: 2,
    cards: Object.freeze([
      card('transition-low-carbon', 'energy', '저탄소 전환', '저탄소 전력 40% 이상을 3일 유지', 3),
      card('transition-economy', 'economy', '성장 여력', '일일 순수익 +4.00 이상을 3일 유지', 3),
      card('transition-carbon', 'environment', '탄소 안전선', 'CO₂ 10/일 이하를 3일 유지', 3),
    ]),
  ...
  specialization: Object.freeze({
    ...
    cards: Object.freeze([
      ...
      card('specialization-grid', 'energy', '효율적 전력망', '충전량 8E의 가동 배터리 또는 송전효율 90%를 3일 유지', 3),
      card('specialization-citizen', 'citizen', '시민과 필수시설', '필수시설 전력 90%·고용률 80%를 3일 유지', 3),
    ]),
  ...
  resilience: Object.freeze({
    ...
    required: 3,
    cards: Object.freeze([
      card('resilience-profit', 'economy', '지속 흑자', '양의 순수익을 4일 유지', 4),
      card('resilience-event-reserve', 'energy', '비상 예비력', `${EVENT_FORECAST_DAYS}일 전 예보에 대비해 이벤트 중 필수시설 90%·저장량 5E를 4일 유지`, 4),
      card('resilience-environment', 'environment', '저탄소 70%·물 제한 이내를 4일 유지', 4),
