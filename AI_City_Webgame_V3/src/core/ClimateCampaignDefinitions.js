...
// 수치(지속일·시설/도시 계수·녹지 흡수·목표·보상)는 settings.json의 CLIMATE_EVENTS / FINAL_CLIMATE_PHASES /
// QUESTS 표에서 id로 읽는다. 여기에는 id·문구·아이콘·목표 종류 같은 구조와 문구만 남긴다.
...
const climateEvent = ({
  ...
  const numbers = settingsRow('CLIMATE_EVENTS', id);
  return Object.freeze({
    ...
    description,
    preparation,
    facilityModifiers: freezeModifiers(numbers.facilityModifiers),
    cityModifiers: Object.freeze({ ...numbers.cityModifiers }),
    greenAbsorptionByLevel: numbers.greenAbsorptionByLevel
      ? Object.freeze([...numbers.greenAbsorptionByLevel])
      : null,
  });
};
...
export const CLIMATE_EVENT_DEFINITIONS = Object.freeze({
  ...
  drought: climateEvent({
    ...
    // 물 한도는 "예보 직전 사용량 그대로"(waterLimitRatio 1.0). 냉각 강화로 늘어난 부담을 상쇄해야 지킬 수 있다.
  }),
...
const climateQuest = ({
  ...
  const numbers = settingsRow('QUESTS', id);
  return Object.freeze({
    index,
    id,
    title,
    ...
    targetDays: numbers.targetDays,
    carbonTarget: numbers.carbonTarget ?? null,
    batteryTarget: numbers.batteryTarget ?? null,
    batteryReserveTarget: numbers.batteryReserveTarget ?? null,
    generationTypeTarget: numbers.generationTypeTarget ?? null,
    tidalEnergyTarget: numbers.tidalEnergyTarget ?? null,
    entry: numbers.entry ? Object.freeze({ ...numbers.entry }) : null,
    reward: reward(numbers.reward),
  });
};
...
const finalPhase = ({ id, label, icon, description = null, preparation = null }) => {
  const numbers = settingsRow('FINAL_CLIMATE_PHASES', id);
  return Object.freeze({
    id,
    label,
    icon,
    ...
    facilityModifiers: freezeModifiers(numbers.facilityModifiers),
    cityModifiers: Object.freeze({ ...numbers.cityModifiers }),
    greenAbsorptionByLevel: numbers.greenAbsorptionByLevel ? Object.freeze([...numbers.greenAbsorptionByLevel]) : null,
  });
};

// 구간 순서는 이 배열이 정한다. settings.json FINAL_CLIMATE_PHASES는 id별 수치 표일 뿐이다.
export const FINAL_CLIMATE_PHASES = Object.freeze([
  finalPhase({ id: 'baseline', label: '기준 측정', icon: 'activity' }),
  finalPhase({ id: 'heatDome', label: '열돔', icon: 'thermometer-sun' }),
  finalPhase({ id: 'monsoonFront', label: '장마전선', icon: 'cloud-rain-wind' }),
  finalPhase({ id: 'coastalSuperstorm', label: '해안 초강풍', icon: 'waves' }),
  finalPhase({ id: 'winterDisaster', label: '겨울 재난', icon: 'snowflake' }),
  finalPhase({ id: 'stagnantAir', label: '대기 정체', icon: 'cloud-fog' }),
  finalPhase({
    id: 'dryEmergency', label: '건조 위기', icon: 'flame',
    description: '도시 고정 탄소가 늘고 냉각 부담이 커지는 동안 물 사용량을 구간 직전 수준 이하로 유지해야 합니다.',
    preparation: '순환냉각을 물 소비가 큰 시설 옆에 연결하고 물 소비가 큰 시설의 강화를 미루세요.',
  }),
  finalPhase({ id: 'recovery', label: '최종 복구', icon: 'heart-pulse' }),
]);
