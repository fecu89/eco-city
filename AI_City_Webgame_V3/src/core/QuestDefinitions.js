...
// 보상 수치(크레딧·해금 시설·해금 연구·강화 허가)는 settings.json QUESTS 표에서 퀘스트 id로 읽는다.
// 퀘스트 순서(index)·제목·목표·세부 문구는 이 파일의 RAW_QUESTS 배열이 정한다.
function quest(id, title, goal, progressKind, details, { quizKind = null } = {}) {
  const { reward: numbers } = settingsRow('QUESTS', id);
  const unlocks = numbers.unlockFacilities ?? [];
  return Object.freeze({
    ...
    reward: Object.freeze({
      credits: numbers.credits,
      unlockFacility: unlocks[0] || null,
      unlockFacilities: Object.freeze([...unlocks]),
      unlockResearch: Object.freeze([...(numbers.unlockResearch ?? [])]),
      upgradePermitLevel: numbers.upgradePermitLevel ?? null,
      upgradePermitFacilities: Object.freeze([...(numbers.upgradePermitFacilities ?? [])]),
      stressTest: numbers.stressTest ?? false,
    }),
  });
}
...
const RAW_QUESTS = [
  quest('first-citizens', '2040, 첫 시민', '주거지 2개를 건설하세요.', 'count', [
    '주거지 2개를 완공하세요.',
    '보상으로 공장과 화력발전을 동시에 해금합니다.',
  ]),
  quest('power-on', '생산 기반을 함께 켜라', '공장과 화력발전을 인접 배치하고 공장을 흑자로 2일 가동하세요.', 'days', [
  ...
  quest('jobs-and-tax', '첫 녹지 조성', '녹지 1칸을 건설하세요.', 'count', [
    '이전 퀘스트 보상으로 해금된 녹지를 1칸 건설하세요.',
    '녹지는 탄소를 낮추고 인접한 주거지의 생활권을 개선합니다.',
    '완료하면 데이터센터를 해금합니다.',
  ]),
  quest('research-seed', '연구도시의 씨앗', '데이터센터 전력 공급률을 90% 이상으로 2일 유지하세요.', 'days', [
    '데이터센터를 건설하세요.',
    '전력이 부족하면 생활 필수 수요 다음으로 데이터센터, 공장 순서로 공급됩니다.',
    '해당 데이터센터의 전력 공급률 90% 이상을 2일 연속 유지하세요.',
  ]),
  quest('growth-cost', '탄소 전환선', '핵발전을 가동해 저탄소 전력 40% 이상, CO₂ 12 이하와 흑자를 2일 유지하세요.', 'days', [
  ...
  quest('water-cycle', '도시 물순환', '데이터센터와 순환냉각을 연결하고 물 사용을 기준 이하로 2일 유지하세요.', 'days', [
  ...
  quest('solar-research-foundation', '태양광 연구 기초', '고효율 태양전지 연구를 완료하세요.', 'research', [
    '태양광을 해금한 뒤 데이터센터에서 고효율 태양전지 연구를 시작하세요.',
    '데이터센터 전력 공급률 90% 이상을 유지하면 연구가 진행됩니다.',
    '아직 맞히지 않은 전용 퀴즈로 남은 연구 시간을 줄일 수 있습니다.',
  ]),
  quest('data-center-modernization', '데이터센터 현대화', '데이터센터 Lv.2와 스마트 전력망 연구를 완성하세요.', 'modernization', [
  ...
  quest('wind-pilot-grid', '풍력 실증망', '풍력 예측 제어를 연구하고 풍력 전력을 2일 연속 공급하세요.', 'days', [
    '풍력발전을 건설한 뒤 풍력 예측 제어 연구를 완료하세요.',
    '풍력발전에서 소비시설로 실제 전력이 전달되어야 합니다.',
    '두 조건을 같은 날 2일 연속 유지하세요.',
  ]),
  quest('tidal-coast-pilot', '해안 조력 실증', '조력 연구와 발전소를 완성하고 조력 전력을 2일 연속 공급하세요.', 'days', [
  ...
  quest('national-climate-test', '대한민국 복합기후 시험', `${stressTestTotalDays()}일 복합 기후 스트레스 테스트를 통과하세요.`, 'stress', [
