...
test('HUD shows only the date while the hourly simulation keeps advancing', async ({ gamePage: page }) => {
  ...
  const pausedHours = await page.evaluate(() => window.__GAME_STATE__.elapsedGameHours);
  ...
  expect(await page.evaluate(() => window.__GAME_STATE__.elapsedGameHours)).toBe(pausedHours);

  await page.evaluate(() => window.__settleSimulationHour());
  expect(await page.evaluate(() => window.__GAME_STATE__.elapsedGameHours)).toBe(pausedHours + 1);
  await expect(page.locator('#simTime')).toHaveText(pausedStart);
});
